...
export default function Cashbox() {
  ...
  const handleExchange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Valyuta ayirboshlashda xatolik yuz berdi');
    }
  };
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.response?.data?.error || 'Xarajat qo\'shishda xatolik yuz berdi');
    }
  };
  ...
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      loadCashbox();
      alert('Трансфер муваффақиятли амалга оширилди!');
    } catch (error: any) {
      alert(error.response?.data?.error || 'Хатолик юз берди');
    }
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(dataToExport, 'Kassa_Tranzaksiyalar', 'Tranzaksiyalar');
  };
  ...
  const handleExportExpensesExcel = () => {
    ...
    exportToExcel(dataToExport, 'Kassa_Xarajatlar', 'Xarajatlar');
  };
  ...
  return (
    <div className="space-y-12 pb-20 animate-in fade-in duration-1000">
      ...
      <div className="relative overflow-hidden bg-white dark:bg-gray-900 rounded-[3rem] p-10 sm:p-16 shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-white dark:border-gray-800">
        <div className="absolute top-0 -left-10 w-64 h-64 bg-emerald-100 dark:bg-emerald-900/20 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-blob pointer-events-none"></div>
        ...
        <div className="relative z-10">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 rounded-full border border-emerald-100 dark:border-emerald-800 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">
              ...
              <h1 className="text-5xl sm:text-7xl font-black text-gray-900 dark:text-white tracking-tighter leading-[0.9]">
                {t("Kassa")} <br />
                <span className="text-emerald-600">{t("Nazorati")}</span>
              </h1>
              <div className="flex flex-wrap gap-4 text-gray-500 dark:text-gray-400">
                <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-2xl font-bold text-sm">
                  <RefreshCw className={`w-4 h-4 text-emerald-500 ${loading ? 'animate-spin' : ''}`} />
                  {loading ? '...' : t("Yangilangan")}
                </div>
                <button 
                  onClick={() => setShowExchangeRateModal(true)}
                  className="flex items-center gap-2 bg-amber-50 dark:bg-amber-900/20 px-4 py-2 rounded-2xl font-bold text-sm text-amber-700 dark:text-amber-400 border border-amber-100 dark:border-amber-800 transition-all hover:scale-105"
                >
            ...
            <div className="flex flex-wrap gap-4 w-full lg:w-auto">
              <button 
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-8 py-5 bg-purple-600 hover:bg-purple-700 rounded-[2rem] font-black text-sm transition-all active:scale-95 text-white shadow-2xl shadow-purple-500/30"
              >
              ...
              <button 
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-10 py-5 bg-emerald-600 hover:bg-emerald-700 rounded-[2rem] font-black text-sm transition-all active:scale-95 text-white shadow-2xl shadow-emerald-500/30"
              >
              ...
              <button 
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-3 px-10 py-5 bg-rose-600 hover:bg-rose-700 rounded-[2rem] font-black text-sm transition-all active:scale-95 text-white shadow-2xl shadow-rose-500/30"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          { 
            ...
            icon: Wallet,
            color: 'amber',
            desc: t("Barcha hisoblar jami"),
            warning: false
          }
        ].map((kpi, idx) => (
          <div 
            key={idx}
            className={`group relative bg-white dark:bg-gray-900 rounded-[2.5rem] p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)] border-2 transition-all duration-500 ${
              kpi.warning ? 'border-rose-200 dark:border-rose-900/50 animate-pulse' : 'border-gray-100 dark:border-gray-800'
            }`}
          >
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 shadow-2xl transition-all duration-500 group-hover:rotate-6 ${
              kpi.color === 'emerald' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30' : 
              kpi.color === 'blue' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30' : 
              kpi.color === 'purple' ? 'bg-purple-50 text-purple-600 dark:bg-purple-900/30' : 'bg-amber-50 text-amber-600 dark:bg-amber-900/30'
            }`}>
              <kpi.icon className="w-7 h-7" />
            </div>
            
            <p className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">{kpi.title}</p>
            <h3 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight mb-2 leading-none break-all">{kpi.value}</h3>
            
            <div className="flex items-center gap-2 mt-4">
              <div className={`w-1.5 h-1.5 rounded-full ${kpi.warning ? 'bg-rose-500' : 'bg-emerald-500'} animate-pulse`}></div>
              <p className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">{kpi.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-8">
        <div className="flex flex-wrap gap-4 p-2 bg-gray-100 dark:bg-gray-800 rounded-[2rem] w-fit">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-3 px-8 py-4 rounded-[1.5rem] font-black text-xs transition-all duration-300 ${
                activeTab === tab.id 
                  ? 'bg-white dark:bg-gray-900 text-emerald-600 shadow-xl scale-105' 
        ...
        <div className="space-y-12 animate-in slide-in-from-bottom-10 duration-700">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
            ...
            <div className="flex flex-wrap gap-3 w-full lg:w-auto">
              <button 
                ...
                className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-2xl font-black text-xs border border-emerald-100 dark:border-emerald-800 transition-all hover:scale-105"
              >
              ...
              <button 
                ...
                className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-gray-50 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-2xl font-black text-xs border border-gray-200 dark:border-gray-700 transition-all hover:scale-105"
              >
                <Settings className="w-4 h-4" />
                {t("SOZLAMALAR")}
              </button>
              <button 
                ...
                className="flex-[2] lg:flex-none flex items-center justify-center gap-3 px-8 py-4 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-black text-xs shadow-xl shadow-rose-500/20 transition-all active:scale-95"
              >
            ...
            {expenseCategories.map((category) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs font-black text-center text-gray-900 dark:text-white uppercase tracking-wider">{category.name}</p>
                  <div className="mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
                    <p className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 text-center">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[3rem] border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-50 dark:border-gray-800 flex justify-between items-center">
              <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">{t("Oxirgi Xarajatlar")}</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-gray-800/50">
                    <th className="text-left py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Sana")}</th>
                    <th className="text-left py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Kategoriya")}</th>
                    <th className="text-left py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("To'lov Usuli")}</th>
                    <th className="text-left py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Tavsif")}</th>
                    <th className="text-left py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Chek №")}</th>
                    <th className="text-right py-5 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Summa")}</th>
                  </tr>
                </thead>
                      ...
                      <tr key={expense.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors group">
                        ...
                        <td className="py-5 px-8">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-wide">{category?.name || expense.category}</span>
                          </div>
                        </td>
                        <td className="py-5 px-8">
                        ...
                        <td className="py-5 px-8">
                          <div className="text-[10px] font-black text-gray-400 font-mono tracking-tighter">
                    ...
                    <tr>
                      <td colSpan={6} className="py-20 text-center">
                        <div className="flex flex-col items-center justify-center opacity-20">
                          ...
                          <p className="font-black uppercase tracking-[0.2em]">{t("Hali xarajatlar mavjud emas")}</p>
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        <div className="space-y-12 animate-in slide-in-from-bottom-10 duration-700">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-emerald-600 to-blue-700 rounded-[3.5rem] p-12 text-white shadow-2xl shadow-emerald-500/20">
            ...
            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10 text-center md:text-left">
              <div className="space-y-4">
                <p className="text-emerald-100 font-black uppercase tracking-[0.3em] text-xs">{t("Jami Kassa Balansi")}</p>
                <h2 className="text-6xl sm:text-8xl font-black tracking-tighter leading-none">
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-800/50 p-8 rounded-[2.5rem] animate-pulse">
              <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                  <h4 className="text-xl font-black text-amber-900 dark:text-amber-100 uppercase tracking-tight mb-2">{t("Kassa Limiti Oshdi!")}</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {cashWarning && (
                      <div className="bg-white/50 dark:bg-black/20 p-4 rounded-2xl border border-amber-100 dark:border-amber-800">
                        <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">{t("Naqd UZS")}</p>
                        <p className="text-lg font-black text-amber-900 dark:text-amber-100">{(cashbox?.byCurrency?.cashUZS || 0).toLocaleString()} so'm</p>
                      </div>
                    )}
                    {cardWarning && (
                      <div className="bg-white/50 dark:bg-black/20 p-4 rounded-2xl border border-amber-100 dark:border-amber-800">
                        <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">{t("Naqd USD")}</p>
                        <p className="text-lg font-black text-amber-900 dark:text-amber-100">${(cashbox?.byCurrency?.cashUSD || 0).toFixed(2)}</p>
                      </div>
                    )}
                    {clickWarning && (
                      <div className="bg-white/50 dark:bg-black/20 p-4 rounded-2xl border border-amber-100 dark:border-amber-800">
                        <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">{t("Click UZS")}</p>
                        <p className="text-lg font-black text-amber-900 dark:text-amber-100">{(cashbox?.byCurrency?.clickUZS || 0).toLocaleString()} so'm</p>
                      </div>
                    )}
                  </div>
                  <button 
                    onClick={() => setShowLimits(true)} 
                    className="mt-6 px-8 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-black text-xs transition-all active:scale-95 shadow-lg shadow-amber-500/20"
                  >
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[3rem] border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-gray-50 dark:border-gray-800 flex justify-between items-center bg-gray-50/30 dark:bg-gray-800/30">
              <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">{t("Oxirgi Tranzaksiyalar")}</h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-gray-800/50">
                    <th className="text-left py-6 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Sana")}</th>
                    <th className="text-left py-6 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Turi")}</th>
                    <th className="text-left py-6 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Usul")}</th>
                    <th className="text-left py-6 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Tavsif")}</th>
                    <th className="text-right py-6 px-8 text-[10px] font-black text-gray-400 uppercase tracking-widest">{t("Summa")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                  {transactions.map((tx) => (
                    <tr key={tx.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors group">
                      ...
                      <td className={`py-6 px-8 text-right font-black text-lg tracking-tight ${
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleAddMoney} className="p-10 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    className="h-16 rounded-2xl border-2 border-gray-100 dark:border-gray-800 focus:border-emerald-500 transition-all font-black text-lg"
                    placeholder="0.00"
                    required
                  />
                </div>
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full h-16 rounded-2xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-6 font-black text-lg focus:border-emerald-500 outline-none transition-all appearance-none"
                  >
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    ...
                    >
                      ...
                      <span className={`text-[10px] font-black uppercase tracking-widest ${form.type === method.id ? `text-${method.color}-700` : 'text-gray-400'}`}>
                        {method.name}
                      </span>
                    </button>
                  ))}
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Tavsif")}</label>
                <textarea
              ...
              <button
                type="submit"
                className="w-full h-16 bg-emerald-600 hover:bg-emerald-700 text-white rounded-3xl font-black text-sm tracking-[0.2em] shadow-2xl shadow-emerald-500/30 transition-all active:scale-95"
              >
                {t("TASDIQLASH")}
              </button>
            </form>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleWithdraw} className="p-10 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <Input
                    ...
                    value={form.amount}
                    onChange={(e) => setForm({ ...form, amount: e.target.value })}
                    className="h-16 rounded-2xl border-2 border-gray-100 dark:border-gray-800 focus:border-rose-500 transition-all font-black text-lg"
                    placeholder="0.00"
                    required
                  />
                </div>
                <div className="space-y-3">
                  ...
                  <select
                    value={form.currency}
                    onChange={(e) => setForm({ ...form, currency: e.target.value })}
                    className="w-full h-16 rounded-2xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-6 font-black text-lg focus:border-rose-500 outline-none transition-all appearance-none"
                  >
                    <option value="USD">USD ($)</option>
                    <option value="UZS">UZS (so'm)</option>
                  </select>
              ...
              <button
                type="submit"
                className="w-full h-16 bg-rose-600 hover:bg-rose-700 text-white rounded-3xl font-black text-sm tracking-[0.2em] shadow-2xl shadow-rose-500/30 transition-all active:scale-95"
              >
                {t("TASDIQLASH")}
              </button>
            </form>
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleTransfer} className="p-10 space-y-8">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Qayerdan")}</label>
                  <select
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Qayerga")}</label>
                  <select
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Summa (USD)")}</label>
                <Input
                  type="text"
                  inputMode="decimal"
            ...
            <div className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Boshlanish")}</label>
                  <input
                    type="date"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Tugash")}</label>
                  <input
                    type="date"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Turi")}</label>
                <select
                  value={filters.type}
                  onChange={(e) => setFilters({ ...filters, type: e.target.value })}
                  className="w-full h-12 rounded-xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-4 font-black text-xs appearance-none"
                >
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Usul")}</label>
                <select
              ...
              <div className="flex gap-3 pt-4">
                <button
                  ...
                  className="flex-1 h-12 rounded-xl border-2 border-gray-100 dark:border-gray-800 font-black text-[10px] uppercase tracking-widest text-gray-500 hover:bg-gray-50 transition-all"
                >
                  {t("TOZALASH")}
                </button>
                <button
                  onClick={applyFilters}
                  className="flex-[2] h-12 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20"
                >
                  {t("QO'LLASH")}
                </button>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleExchange} className="p-10 space-y-8">
              <div className="bg-amber-50 dark:bg-amber-900/20 p-6 rounded-[2rem] border border-amber-100 dark:border-amber-800 text-center">
                <p className="text-xs font-black text-amber-800 dark:text-amber-400 uppercase tracking-widest">
                  💱 {t("JORIY KURS")}: 1 USD = {exchangeRate.toLocaleString()} so'm
                </p>
              </div>

              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("QAYSI VALYUTADAN")}</label>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("QAYSI VALYUTAGA")}</label>
                  <div className="grid grid-cols-2 gap-2">
                    {['USD', 'UZS'].map(curr => (
                      <button
                        key={curr}
              ...
              <div className="space-y-3">
                ...
                <Input
                  ...
                  className="h-16 rounded-2xl border-2 border-gray-100 dark:border-gray-800 focus:border-purple-500 transition-all font-black text-lg"
                  placeholder="0.00"
                  required
                />
              </div>
              ...
              {exchangeForm.amount && (
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-3xl border border-emerald-100 dark:border-emerald-800 flex flex-col items-center justify-center gap-2">
                  <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">{t("HISOBLANGAN NATIJA")}</p>
                  <p className="text-2xl font-black text-emerald-900 dark:text-emerald-100 tracking-tighter">
      ...
      {showExchangeRateModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">💵 1 USD = ? UZS</label>
                <input
                  type="text"
                  inputMode="decimal"
                  ...
                  className="w-full h-16 rounded-2xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-6 font-black text-2xl text-center focus:border-amber-500 transition-all outline-none"
                />
              </div>
              <button
                onClick={saveExchangeRate}
                className="w-full h-14 bg-gray-900 dark:bg-white dark:text-black text-white rounded-2xl font-black text-xs tracking-widest transition-all active:scale-95 shadow-xl"
              >
                {t("SAQLASH")}
              </button>
            </div>
            ...
            <div className="p-10 space-y-8">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-xs font-black uppercase tracking-widest">{t("OGOHLANTIRISHLAR")}</span>
                </div>
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("NAQD PUL LIMITI (USD)")}</label>
                  <Input
                    type="text"
                    inputMode="decimal"
                    ...
                    className="h-14 rounded-xl border-2 border-gray-100 dark:border-gray-800 font-black"
                    disabled={!limits.alertEnabled}
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("KARTA LIMITI (USD)")}</label>
                  <Input
                    type="text"
                    inputMode="decimal"
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("CLICK LIMITI (USD)")}</label>
                  <Input
                    type="text"
                    inputMode="decimal"
              ...
              <button
                ...
                className="w-full h-16 bg-gray-900 dark:bg-white dark:text-black text-white rounded-[2rem] font-black text-xs tracking-[0.2em] shadow-2xl transition-all active:scale-95"
              >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xl flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            ...
            <form onSubmit={handleAddExpense} className="p-10 space-y-8 overflow-y-auto">
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("KATEGORIYA TANLANG")}</label>
                  ...
                  {expenseCategories.map((category) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-black uppercase tracking-tight text-left leading-tight">{category.name}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full h-16 rounded-2xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-6 font-black text-sm focus:border-rose-500 outline-none transition-all appearance-none"
                  >
                    <option value="CASH">💵 {t("Naqd")}</option>
                    <option value="CARD">💳 {t("Karta")}</option>
                    <option value="CLICK">📱 {t("Click")}</option>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("Sana")}</label>
                  <input
                    type="date"
                    value={expenseForm.date}
                    onChange={(e) => setExpenseForm({ ...expenseForm, date: e.target.value })}
                    className="w-full h-16 rounded-2xl border-2 border-gray-100 dark:bg-gray-800 dark:border-gray-800 px-6 font-black text-sm outline-none"
                  />
                </div>
              </div>
              ...
              <div className="bg-rose-50 dark:bg-rose-900/20 p-6 rounded-3xl border border-rose-100 dark:border-rose-800 flex items-center gap-4">
                ...
                <p className="text-xs font-black text-rose-900 dark:text-rose-100 uppercase tracking-tight leading-tight">
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            ...
            <div className="p-10 space-y-10 overflow-y-auto">
              <div className="bg-gray-50 dark:bg-gray-800/50 p-8 rounded-[2.5rem] space-y-6">
                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("YANGI QO'SHISH")}</h4>
                <div className="space-y-6">
                  <Input
                  ...
                  />
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("RANG TANLANG")}</label>
                  ...
                  <button 
                    ...
                    className="w-full h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-xs tracking-widest shadow-xl shadow-emerald-500/20"
              ...
              <div className="space-y-6">
                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">{t("MAVJUD KATEGORIYALAR")}</h4>
                    ...
                    return (
                      <div key={category.id} className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 border-2 border-gray-100 dark:border-gray-800 rounded-2xl group transition-all hover:border-gray-200">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">{category.name}</p>
                            {isDefault && <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">{t("STANDART")}</p>}
                          </div>
                        </div>
                        {!isDefault && (
                          <button
