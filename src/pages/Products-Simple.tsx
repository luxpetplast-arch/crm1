...
export default function Products() {
  ...
  // Mahsulotlarni turiga bo'lish guruhlash
  const groupProductsByType = (products: any[]) => {
    const grouped: Record<string, any[]> = {};
    
    products.forEach(product => {
      const type = product.bagType || 'Noma\'lum';
      if (!grouped[type]) {
        grouped[type] = [];
      }
  ...
  const productTypes = Object.keys(groupedProducts).sort();
    ...
    <div className="p-4 max-h-screen overflow-y-auto">
              ...
              {step === 1 ? (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Input 
                      label="Qop Turi *" 
                      placeholder="Masalan: 15mg-QORA, 30mg-OQ"
                      value={form.bagType} 
      ...
      {productTypes.map((type) => (
        <div key={type} className="mb-8">
          {/* Turkacha sarlavha */}
          <div className="flex items-center gap-3 mb-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 rounded-xl border-2 border-blue-200 dark:border-blue-800">
            <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg">
              <Package className="w-6 h-6 text-white" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-blue-700 dark:text-blue-300">{type}</h2>
              <p className="text-sm text-blue-600 dark:text-blue-400">
                {groupedProducts[type].length} mahsulot • {groupedProducts[type].reduce((sum, p) => sum + p.currentStock, 0)} qop
              </p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <CardContent className="p-4">
                    {/* Header - Faqat asosiy ma'lumotlar */}
                    ...
                    <div className={`mb-3 px-3 py-2 rounded-lg ${status.bgColor} flex items-center justify-between`}>
                      ...
                      <span className="font-bold text-lg">{product.currentStock} qop</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      ...
                      <div className="bg-blue-50 dark:bg-blue-950 p-3 rounded-lg">
                        <p className="text-xs text-blue-600 dark:text-blue-400 mb-1">💰 Narx/qop</p>
                        <p className="font-bold text-lg text-blue-700 dark:text-blue-300">
                          {(product.pricePerBag / 1000).toFixed(0)}K so'm
                        </p>
                      </div>
                      
                      {/* Dona */}
                      <div className="bg-green-50 dark:bg-green-950 p-3 rounded-lg">
                        <p className="text-xs text-green-600 dark:text-green-400 mb-1">📦 Dona/qop</p>
                        <p className="font-bold text-lg text-green-700 dark:text-green-300">
                          {product.unitsPerBag} dona
                        </p>
                      </div>
                    </div>
