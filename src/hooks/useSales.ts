...
export const useSales = () => {
  ...
  const filteredSales = useMemo(() => {
    return sales.filter((sale) => {
      ...
      if (filter.status === 'paid') return !sale.debt || sale.debt === 0;
      if (filter.status === 'debt') return sale.debt && sale.debt > 0;
  ...
  const stats: SalesStats = useMemo(() => {
    return {
      ...
      totalDebt: sales.reduce((sum, s) => sum + (s.debt || 0), 0),
      paidCount: sales.filter((s) => !s.debt || s.debt === 0).length,
      debtCount: sales.filter((s) => s.debt && s.debt > 0).length,
    };
  }, [sales]);
