  ...
  const formatText = (text: string) => {
    return text.split('\n').map((line, i) => {
      if (line.startsWith('1. **') || line.startsWith('2. **') || line.startsWith('3. **') || line.startsWith('4. **') || line.startsWith('5. **')) {
        return <h3 key={i} className="text-xl font-bold text-blue-600 mt-8 mb-4 flex items-center gap-2">
        ...
        <div className="relative z-10">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-50 dark:bg-blue-900/30 rounded-full border border-blue-100 dark:border-blue-800 text-[10px] font-semibold uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400">
            ...
            <Button 
              ...
              disabled={loading}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-sm shadow-lg shadow-blue-500/30 transition-all hover:scale-105 active:scale-95 flex items-center gap-2"
            >
        ...
        {error ? (
          <Card className="border-rose-200 bg-rose-50 dark:bg-rose-900/10 dark:border-rose-900/30">
            <CardContent className="p-10 text-center">
              ...
              <h3 className="text-2xl font-bold text-rose-600 mb-4">{latinToCyrillic("Xatolik!")}</h3>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-[4rem] border border-gray-100 dark:border-gray-800 shadow-2xl overflow-hidden">
            <div className="p-10 sm:p-16">
              <div className="flex items-center gap-4 mb-12 pb-8 border-b border-gray-50 dark:border-gray-800">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">{latinToCyrillic("AI Ekspert Xulosasi")}</h2>
                  <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">{new Date().toLocaleDateString()} holatiga ko'ra</p>
                </div>
              </div>
