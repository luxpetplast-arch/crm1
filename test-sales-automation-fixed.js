...
const testData = {
  customer: {
    name: `Test Mijoz Avtomatik ${Date.now()}`,
    phone: `+99890123456${Math.floor(Math.random() * 10)}`,
    address: 'Toshkent, Chilonzor',
    telegramChatId: `${Date.now()}`,
    telegramUsername: `test_customer_${Date.now()}`
  },
  product: {
    name: `Test Mahsulot Avtomatik ${Date.now()}`,
    bagType: 'KICHIK',
    unitsPerBag: 50,
    minStockLimit: 10,
    optimalStock: 50,
...
async function testCompleteSalesAutomation() {
  ...
  try {
    ...
    console.log('\n   📦 OMBOR HOLATI:');
    const updatedProductResponse = await apiWithAuth.get(`/products/${product.id}`);
    ...
    console.log('\n   📊 STOCK MOVEMENT:');
    try {
      const stockMovementsResponse = await apiWithAuth.get(`/products/${product.id}/stock-movements`);
      ...
      console.log(`   ⚠️ StockMovement API mavjud emas`);
    }

    // 4.3 Kassa tranzaksiyasi yaratilganini tekshirish
    console.log('\n   💰 KASSA TRANZAKSIYASI:');
    try {
      const cashboxResponse = await apiWithAuth.get('/cashbox/transactions?limit=10');
      ...
      console.log(`   ⚠️ Kassa API xatolik: ${error.response?.status}`);
    }
    ...
    console.log('\n   💳 MIJOZ QARZ:');
    const updatedCustomerResponse = await apiWithAuth.get(`/customers/${customer.id}`);
...
async function testLowStockAlert() {
  ...
  try {
    ...
    const productData = {
      name: `Test Low Stock Mahsulot ${Date.now()}`,
      bagType: 'KICHIK',
      unitsPerBag: 50,
      minStockLimit: 20,
      optimalStock: 50,
    ...
    const productResponse = await apiWithAuth.post('/products', productData);
    const product = productResponse.data;
    ...
    const customerResponse = await apiWithAuth.post('/customers', {
      name: `Test Low Stock Mijoz ${Date.now()}`,
      phone: `+99890999999${Math.floor(Math.random() * 10)}`,
      telegramChatId: `${Date.now() + 1000}`
    });
    const customer = customerResponse.data;
