  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.data?.token) {
        ...
        const branchId = data.data.user?.branch?._id || data.data.user?.branch
        const branchName = data.data.user?.branch?.name || 'Filial'
        ...
        localStorage.setItem('cashierToken', data.data.token)
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center p-4 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-teal-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse dark:bg-teal-600 dark:opacity-10"></div>
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-cyan-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse dark:bg-cyan-600 dark:opacity-10" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/2 left-1/2 w-96 h-96 bg-green-500 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse dark:bg-green-600 dark:opacity-10" style={{ animationDelay: '4s' }}></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="card-glass backdrop-blur-2xl rounded-3xl p-8 shadow-2xl">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-500 shadow-2xl mb-4">
              <Smartphone className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-4xl font-black text-[var(--text-primary)] mb-2">F-Mobile</h1>
            <p className="text-[var(--text-secondary)]">Kassir Panel</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="btn-primary w-full py-3 rounded-xl font-semibold transition-all duration-300 transform flex items-center justify-center gap-2 hover:shadow-2xl hover:shadow-teal-500/50 disabled:opacity-50 hover:scale-105 active:scale-95"
