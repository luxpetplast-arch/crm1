...
function importProducts() {
    console.log('📦 Mahsulotlarni import qilish boshlanmoqda...');
    
    try {
        // Avval eski mahsulotlarni o'chirish
        ...
        products.forEach((product, index) => {
            try {
                ...
                // To'liq 13 ta qiymat
                const query = `INSERT INTO [Product] (id, name, bagType, unitsPerBag, minStockLimit, optimalStock, maxCapacity, currentStock, currentUnits, pricePerBag, productionCost, createdAt, updatedAt) VALUES ('${id}', '${product.name}', '${product.bagType}', ${product.unitsPerBag}, 50, 500, 0, 0, ${product.price}, 0.0, datetime('now'), datetime('now'));`;
                
                execSync(`sqlite3 prisma/dev.db "${query}"`, { encoding: 'utf8' });
                importedCount++;
                
                console.log(`✅ ${importedCount}. ${product.name} - ${product.unitsPerBag} dona/qop`);
        ...
        // Tekshirish
        const result = execSync('sqlite3 prisma/dev.db "SELECT COUNT(*) as count FROM [Product];"', { encoding: 'utf8' });
        ...
        const sampleResult = execSync('sqlite3 prisma/dev.db "SELECT name, bagType, unitsPerBag, pricePerBag FROM [Product] LIMIT 5;"', { encoding: 'utf8' });
